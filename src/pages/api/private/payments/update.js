...
import { updatePayment } from '../../../../server/services/Payments/paymentsService.js';
...
export default async function update(req, res) {
  ...
  if(isNaN(total)){ 
    config.sendError(res, 403, 'Qs Not is numbers');
    return false;
  }
  ...
  const data = await updatePayment(req.body.id, values);
